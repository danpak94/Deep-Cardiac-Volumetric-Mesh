...
class Net(nn.Module):
    def __init__(self, params):
        ...
        self.mtm_get_seg_output = params.mtm_get_seg_output
        ...
        # template_filepath = os.path.join('../template_for_deform', '{}_seg.npy'.format(params.template_P_phase))
        # seg_template_np = np.load(template_filepath)
        # self.seg_template = torch.tensor(seg_template_np, dtype=torch.get_default_dtype(), device=device).repeat(self.n_batch,1,1,1,1)
        ...
        self.faces_list_template_torch = utils_sp.np_list_to_torch_list(self.faces_list_template, n_batch=self.n_batch, device=device)
        ...
        self.ds3_1x1_conv3d = nn.Conv3d(self.base_n_filter*4, self.n_channels_out*self.n_dim, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def forward(self, x):
        ...
        # if self.reversed_field and self.mtm_get_seg_output:
        #     # only able to get seg_output if field is reversed
        #     # trying not reversed b/c we can't interpolate if reversed, only possible to use nearest neighbor
        #     grid = F.affine_grid(torch.eye(4, device=displacement_field.device).repeat([self.n_batch, 1, 1])[:,0:3,:], logits.shape, align_corners=True)
        #     field_temp = displacement_field_tuple[0].permute(0,2,3,4,1)
        #     seg_output = F.grid_sample(self.seg_template, grid + field_temp, align_corners=True)
        #     # seg_output = None
        # else:
        #     seg_output = None
        seg_output = None
        
        interp_field_list = utils_sp.interpolate_rescale_field_torch(displacement_field_tuple, self.verts_list_template_torch, img_size=x.shape[2:], reversed_field=False)

        verts_list_transformed_torch = utils_sp.move_verts_with_field(self.verts_list_template_torch, interp_field_list)

        # !!!!! for inference
        # displacement_field_tuple = (self.transformation.get_inverse_displacement(cp_grid),)

        return [seg_output, verts_list_transformed_torch, displacement_field_tuple], [self.verts_list_template_torch, self.faces_list_template_torch, self.extra_info_template]
